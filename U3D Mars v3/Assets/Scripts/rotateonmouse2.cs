...
public class rotateonmouse2 : MonoBehaviour
{
	...
	public GameObject verouille;
	public GameObject debloque;
...
//	public Text txtPlay;
	...
	void OnGUI(){
		...
		//	link.SetActive(false);
			debloque.SetActive(false);
			verouille.SetActive(false);
			next.SetActive(false);
			choose.SetActive(true);

		}else if(currentZone == Zones.blue){

			locker.SetActive(false);
			debloque.SetActive(true);
			verouille.SetActive(false);
		//	link.SetActive(true);
	...
	void MouseUp()
	{
		...
		if (gameManager != GameManager.Zoom) {
			...
			/*	if (txtZone.text == "Région "+ hit.collider.gameObject.tag){
				...
				//	txtZone.color = GetColor(currentZone.ToString());
					...
					txtZone.text =  GetTitle(currentZone.ToString()); //hit.collider.gameObject.tag;
			//		txtZone.color = GetColor(currentZone.ToString());
	...
	void LateUpdate()
	{
		...
		if (gameManager != GameManager.Zoom) {
			...
			if((gesture == GestureTypes.MouseDown) || (gesture == GestureTypes.MouseUp)){ 
				...
				//recenter cam on ZoomIN
							
				//float[] retVal = CartesianToSpherical(camTarget.transform.position.x, camTarget.transform.position.y, camTarget.transform.position.z);
				//toRotation = Quaternion.Euler(retVal[0], retVal[1], 0);
	...
	public string GetTitle(string aColor)
	{
		switch (aColor) {
		case "blue":
			return "Zone d'amarsissage"; 
		case "brown":
			return "Région d'Elysium";    
		case "red":
			return "Région de Memonia";
		case "orange":
			return "Région d'Arcadia";
		case "grey":
			return "Région d'Eridania";
		case "white":
			return "Région d'Iapigya";
		default:
			return "";
		}
